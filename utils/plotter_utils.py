...
from datasets.llm_measurements.query_llm import *


# estimated total cost
def print_estimated_cost(model_cache_file=model_cache_file):
...
def create_grouped_observables_df(df):
    df = df.copy()
    for name, group in speaker_variables_groups.items():
    ...
    for name, group in slice_variables_groups.items():
        group_detailed_names = [g.detailed_name for g in group if g.detailed_name in df.columns]
        if len(group_detailed_names) == 0:
            continue
...
def save_given_observables(slices_d, caption, label, subset=None, file_name='defined_observables_description'):
    global contextual_variables_descriptions
    ...
    for do in defined_observables:
        ...
        assert(do in contextual_variables_descriptions)
    s = []
    for name, des in contextual_variables_descriptions.items():
